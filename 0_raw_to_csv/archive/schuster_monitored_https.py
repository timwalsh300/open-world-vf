...
def parse(subdir):
    files = 0
    periods = SECONDS * PPS
    region = REGION_RE.search(subdir[44:]).group()[:-6]
    output_csv = open('/home/timothy.walsh/VF/schuster_monitored_https/' + region + '_schuster_monitored_https.csv', 'w')
                    ...
                    for timestamp, buf in input_pcap:
                        ...
                        if isinstance(transport, dpkt.udp.UDP):
                            if ip.len > 28:
                                ...
                                # increment bytes for the appropriate period
                                conversations[distant_end][period] += ip.len - 28
                except:
                    print('bad pcap:', path)
                    continue
