        ...
        for trial in range(1):
            ...
            for epoch in range(240):
                ...
                for _ in range(len(train_mon_loader)):
                    ...
                    x_train_mon_features = model.extract_features_gap(x_train_mon.to(device))
                    # label all of these as positive / monitored
                    y_train_mon = torch.ones(len(y_train_mon), dtype=torch.float32)
                    
                    train_unmon = next(train_unmon_iter)
                    x_train_unmon, y_train_unmon = train_unmon
                    x_train_unmon_features = model.extract_features_gap(x_train_unmon.to(device))
                    # label all of these as negative / unmonitored
                    y_train_unmon = torch.zeros(len(y_train_unmon), dtype=torch.float32)
                    ...
                    y_fake_zeros = torch.zeros(len(fake_features), dtype=torch.float32)
                    
                    # train discriminator on real monitored, real unmonitored,
                    # and fake monitored all at once
                    ...
                    loss = criterionG(output, y_fake_ones.to(device))
                    x_train_mon_features = model.extract_features_gap(x_train_mon.to(device))
                    feature_loss = torch.norm(x_train_mon_features.squeeze().mean(dim=0) - features.squeeze().mean(dim=0), p=2)
                ...
                with torch.no_grad():
                    netD.eval()
                    netG.eval()
                    noise = torch.randn(50, 100, 1, 1, device=device)
                    fakes = netG(noise)
                    mean_pred_fake = netD(fakes).squeeze().mean()
                    
                    for x_val, y_val in val_loader:
                        netD.eval()
                        features_val = model.extract_features_gap(x_val.to(device))
                        preds_val = netD(features_val).squeeze()
                        y_val_binary = 1 - y_val[:, 60]
                        loss = criterionD(preds_val, y_val_binary.to(device))
                        val_loss += loss.item()
                        
                        # compute PR-AUC over the validation set
                        precisions, recalls, thresholds = precision_recall_curve(y_val_binary.cpu(), preds_val.cpu())
