...
def create_model(representation, protocol, platform):
    if representation == 'sirinam_wf':
        model = mymodels.DFNet.build(SHAPES[representation], 10)
        lr = 0.002
    else:
        model = mymodels.DFNetTunable.build(SHAPES[representation], 10,
...
def train_model(representation, protocol, platform):
    ...
    with open('../1_csv_to_pkl/' + mod_rep + '_monitored_' + protocol + '_' + platform + '_math.pkl', 'rb') as handle:
