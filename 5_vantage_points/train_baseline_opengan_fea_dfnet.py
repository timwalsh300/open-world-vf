...
BEST_HYPERPARAMETERS = {'schuster8_tor': {'filters': 256, 'kernel': 8, 'conv_stride': 2, 'pool': 8, 'pool_stride': 1, 'conv_dropout': 0.1, 'fc_neurons': 128, 'fc_init': 'he_normal', 'fc_activation': 'relu', 'fc_dropout': 0.4, 'lr': 0.00014940538775594852, 'batch_size': 128},
...
for region in ['oregon']:
    ...
    #for protocol in ['https', 'tor']:
        try:
            ...
            if region == 'oregon':
                train_mon_tensors = torch.load('../4_open_world_enhancements/' + representation + '_' + protocol + '_train_mon_tensors.pt')
            else:
                train_mon_tensors = torch.load('data_splits/train_' + representation + '_' + protocol + '_vimeo_' + region + '.pt')
            train_mon_dataset = torch.utils.data.TensorDataset(*train_mon_tensors)
            train_unmon_tensors = torch.load('../4_open_world_enhancements/' + representation + '_' + protocol + '_train_unmon_tensors.pt')
        ...
        for trial in range(10):
            ...
            model.load_state_dict(torch.load('models/' + representation + '_' + protocol + '_vimeo_' + region + '_model'  + str(trial) + '.pt'))
            model.to(device)
            model.eval()     
            ...
            early_stopping = EarlyStopping(patience = 20,
                                           verbose = True,
                                           path = ('models/' + representation + '_' + protocol + '_' + region + '_opengan_model' + str(trial) + '.pt'))
            ...
            print('Starting to train now for', representation, protocol, region, trial)
